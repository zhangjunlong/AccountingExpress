...
public class UserDaoJpaImpl extends GenericJpaDaoBean<User, Long>
		implements UserDao {
	...
	public List<User> read() throws Exception {
		String ql = "select u from User u order by u.id";

		return this.readSpecifiedSection(ql, 0, 1).getResultList();
	}

	@Override
	public SectionResult<User> read(int size, int section) throws Exception {
		String ql = "select u from User u order by u.id";
